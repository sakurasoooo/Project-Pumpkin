...
{	
	...
	public class Jumper : CorgiMonoBehaviour 
	{
		...
		protected CharacterGlide _characterGlide;
		...
		protected virtual void LateUpdate()
		{
			if (_controller != null)
			{
				...
				_characterGlide = _controller.gameObject.MMGetComponentNoAlloc<Character>()?.FindAbility<CharacterGlide>();
				if (_characterJump != null)
				{
					_characterJump.SetCanJumpStop(false);
					_characterJump.SetJumpFlags();
				}
				if (_characterGlide != null)
				{
					_characterGlide.GlideStop();
				}
